...
This skill runs *before* `systematic-debugging`. Debugging requires knowing which code to debug. Analysis establishes that.
...
**Do NOT use when:** test failure in front of you, stack trace you triggered, error you can reproduce locally. Those go straight to `systematic-debugging`.
...
### Phase 4: Hand Off to `systematic-debugging`
...
**This is the input `systematic-debugging` assumes.** Invoke that skill for the code-level root cause. Don't reinvent its phases here.
...
| **4. Handoff** | Invoke `systematic-debugging` | Has input it assumes |
