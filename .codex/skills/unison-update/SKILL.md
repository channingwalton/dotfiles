...
Triggered when the available Unison `update-definitions` tool returns affected definitions that no longer typecheck.

## Context

When the Unison tool returns `sourceCodeUpdates` containing:
