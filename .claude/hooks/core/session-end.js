 ...
 */
function deleteMemory(endpoint, apiKey, contentHash) {
    return new Promise((resolve, reject) => {
        const url = new URL(`/api/memories/${contentHash}`, endpoint);
        const isHttps = url.protocol === 'https:';
        const requestModule = isHttps ? https : http;
        ...
        req.on('timeout', () => {
            req.destroy();
            resolve({ success: false, error: 'Delete request timed out' });
        });
    ...
    try {
        ...
        if (result.success || result.content_hash) {
            ...
            if (result.content_hash) {
                ...
                // Quality pruning: evaluate and delete if below threshold
                const pruningConfig = config.memoryService?.qualityPruning || {};
                const pruningEnabled = pruningConfig.enabled !== false;
                const minQuality = pruningConfig.minQualityThreshold || 0.4;

                if (pruningEnabled) {
                    try {
                        const evalResult = await triggerQualityEvaluation(endpoint, apiKey, result.content_hash);

                        if (evalResult.success && evalResult.quality_score !== undefined) {
                            const score = evalResult.quality_score;
                            console.log(`[Memory Hook] Quality evaluated: ${score.toFixed(3)} (${evalResult.quality_provider})`);

                            if (score < minQuality) {
                                // Prune low-quality memory
                                const deleteResult = await deleteMemory(endpoint, apiKey, result.content_hash);
                                if (deleteResult.success !== false) {
                                    console.log(`[Memory Hook] Pruned low-quality session (${score.toFixed(3)} < ${minQuality})`);
                                } else {
                                    console.warn(`[Memory Hook] Failed to prune low-quality session: ${deleteResult.error}`);
                                }
                            } else {
                                console.log(`[Memory Hook] Session retained (quality ${score.toFixed(3)} >= ${minQuality})`);
                            }
                        } else {
                            // Evaluation failed - keep the memory (fail-open)
                            console.warn('[Memory Hook] Quality evaluation failed, keeping memory');
                        }
                    } catch (err) {
                        // Don't fail the hook if quality evaluation fails
                        console.warn('[Memory Hook] Quality evaluation error:', err.message);
                    }
                } else {
                    console.log('[Memory Hook] Quality pruning disabled');
                }
            }
        } else {
